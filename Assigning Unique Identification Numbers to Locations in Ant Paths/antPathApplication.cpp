/* 
   antPathApplication.cpp - application file for a program named anthPath
   =====================================================================
...
int main() {
   ...
   /* read the number of test cases from a file */
	...
	i = 0; //This is used as the index and track unique items

	fprintf(fp_out, "jwuyeh\n"); //put andrew ID in file
	if(number_of_test_cases>0){
		//Initialize array for defensive programming
	...
	while (fscanf(fp_in, "%f %f %f", &t, &x, &y) > 0)/* fsanf returns values corresponding to line that's read*/
	{
		cur_x = int(x);
		cur_y = int(y);
		if( t!=-1) //check if time isn't -1
		{

			if( !(foundDuplicate(lcs, i, cur_x, cur_y))) //use the helper function to find duplicates
			{
				lcs[i].x = cur_x;
				lcs[i].y = cur_y;
				i++; //index incremented after the points have been added
				fprintf(fp_out,"%3d    %6.3f %4d %4d\n", i, t, cur_x, cur_y);
			}
		}
		else
		{
			//Restart index (and counting of unique number)
			i= 0;
			fprintf(fp_out, "--------------\n");
		}
	}
	printf("Result in 'output.txt' in data folder.\n"); 
