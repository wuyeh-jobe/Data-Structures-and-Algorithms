...
void main() {
	...
	/** declare binary trees and element **/
	ELEMENT_TYPE ed, ea;
	BINARY_TREE_TYPE dictionaryTree, analysisTree;
	initialize(&dictionaryTree, true); //intialize is set to true for first call
	initialize(&analysisTree, true); //intialize is set to true for first call
	...
	end_of_file = fscanf(fp_in, "%s", filename);  // read a filename from the input file
	...
	while (end_of_file != EOF) {
		...
		checkWhiteSpace (fp_in2, original_word);
		...
		while (end_of_file2 != EOF) {
			...
			if ( stringLength > 0) { /* Check word length is greater than zero before putting it in the tree*/
				
				if (input_dictionary) {
					//Ignore trailing end of line or space
					word[stringLength-1] ='\0';
					/*** building the dictionary ***/
					assign_element_value(&ed, word);
					insert(ed, &dictionaryTree);

				}
				else {
					/* Remove unwanted characters */
					removeUnwantedChars(word);

					/*** analyzing text ***/
					assign_element_value(&ea, word); //read the word

					if (!find(ea, dictionaryTree)){ /* Check if the word is not in dictionary */
						toUpper(original_word);		/* Word is not, convert it to uppercase   */
					}
					else{
						/* The word is in the dictionary, so add it to the analysis tree */
						insert(ea, &analysisTree);
					}
			...
			end_of_file2 = fscanf(fp_in2, "%s", original_word); //read next word in the files
		...
		if (!input_dictionary) {
			/** Check if last original word has end of line character at the end and add if not  **/
			if (!strchr(original_word,'\n')){
				fprintf(fp_out,"\n");
			}

			/** Calculate the height of the tree and print out to the file as the maximum **/
			/** number of probes **/
			fprintf(fp_out, "Maximum number of probes: %d\n", treeHeight(analysisTree));

			/** Calculates total probes and the number of words in a tree **/
			totalProbes(analysisTree);
			/**  Calculate the average from after the total probes and number of words have been calculated **/
			averageProbes = getAverageProbes();
			/** Print out the average number of probes to the output file **/
			fprintf(fp_out, "Average number of probes: %3.1f\n", averageProbes);
			/** Print the words with it's frequency and level in the tree **/
			print_inorder_to_file(analysisTree,fp_out, 0);
			fprintf (fp_out,"--------------------\n");
		}

		fclose(fp_in2);
		end_of_file = fscanf(fp_in, "%s", filename); // read the next filename
		initialize(&analysisTree, false); //false used when initializing for second time
