...
Wuyeh Jobe
29 March 2020
Added function to convert path vertices to cell coordinates and mark them on the graph
...
bool find_path2(int start, int end, int parents[], int *map[], int map_dimension_x, int map_dimension_y) {
	...
	if (end == -1) {
		...
		is_path = find_path2(start,parents[end],parents,map,map_dimension_x, map_dimension_y);
		coordinates(end, map_dimension_x, map_dimension_y, &coord_x, &coord_y);
		(*(map+coord_x))[coord_y] = 4;
		if (debug) printf(" %d",end);
	}
	return(is_path); 
}
...
/** Find the path from start to goal and change the values in the map for writing purposes **/
bool find_path_and_trace(graph *g, int start, int end, int *map[], int map_dimension_x, int map_dimension_y){
	bool is_path;
	...
	if ((start < 1) || (start > g->nvertices)) {
		...
		initialize_search(g);
		bfs(g, start);
		is_path = find_path2(start, end, parent,map,map_dimension_x, map_dimension_y); // now call the version of find_path that has the parent array as an argument
		if (debug) printf("\n");
	}
	return(is_path);
}
